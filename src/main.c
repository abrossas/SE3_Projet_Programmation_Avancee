...
int main ()
{
    ...
    printf ("Bonjour, vous pouvez rentrer les différentes requêtes et terminer en écrivant "
            "quit\nVous pouvez taper help pour afficher les informations sur les différentes "
    ...
    { // Tant que l'utilisateur n'a pas rentré quit
        ...
        if (strcmp (requete, "help\n") == 0) // Help pour afficher les requêtes disponibles, ce qu'elles font et comment les utiliser
        {
            ...
            "-------------------------------------------------------------- HELP "
            "---------------------------------------------------------------\nshow-airports "
            "<airline_id> : affiche tous les aéroports depuis lesquels la compagnie aérienne "
            "<airline_id> opère des vols\n- show-airlines <port_id> : affiche les compagnies "
            "aériens qui ont des vols qui partent de l'aéroport <port_id>\n- show-flights "
            "<port_id> <date> [<time>] [limit=<xx>] : affiche les vols qui partent de l'aéroport à "
            "la date, avec optionnellement une heure de début, et limité à xx vols (les arguments "
            "optionnels ne seront malheureusement pas pris en compte\n- most-delayed-flights : "
            "donne les 5 vols qui ont subi les plus longs retards à l'arrivée\n- "
            "most-delayed-airlines : donne les 5 compagnies aériennes qui ont, en moyenne, le plus "
            "de retards\n- delayed-airline <airline_id> : donne le retard moyen de la compagnie "
            "aérienne passée en paramètre\n- most-delayed-airlines-at-airport <airport_id> : donne "
            "les 3 compagnies aériennes avec le plus de retard d'arrivée à l'aéroport passé en "
            "paramètre\n- changed-flights <date> : les vols annulés ou déviés à la date  (format "
            "M-D)\n- avg-flight-duration <port_id> <port_id> : calcule le temps de vol moyen entre "
            "deux aéroports\n- find-itinerary <port_id> <port_id> <date> : trouve un vol direct "
            "entre les aéroports passés en paramètre à la date voulue\n");
        }
